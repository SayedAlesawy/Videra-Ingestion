...
class HeartBeat(Thread):
    ...
    def intialize_master_connection(self):
        ...
        logger.info(
            f'[{self.tag}] Connection established successfully with execution manager at port {self.master_port}'
        )
        return socket
    ...
    def run(self):
        ...
        while time.sleep(self.update_frequency) or not self.gracefull_shutdown:
            try:
                ...
                logger.error(
                    f'[{self.tag}] Failed to send heartbeat to execution manager on port {self.master_port} due to: {e}'
                )

        self.socket.disconnect(f"tcp://{self.master_ip}:{self.master_port}")
        logger.info(f'[{self.tag}] shutting down hearbeat')
